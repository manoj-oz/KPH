...
const { isAuthenticated, hasAccess } = require('../middleware/checkAccess');
...
module.exports = (pool) => {
  ...
  // ✅ GET /api/enquiries - Fetch all enquiry records (with auth + access check)
  router.get('/enquiries', isAuthenticated, hasAccess('enquiry'), async (req, res) => {
    try {
      const result = await pool.query(
        `SELECT 
          enquiry_id, full_name, phone, email, dob,
